...
<main class="flex-1 p-6 custom-scrollbar overflow-y-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Activity Management</h1>
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <img class="h-12 w-20 object-cover rounded-md border border-gray-200"
                                src="{{ asset('storage/' . ($item->icon ?? 'placeholders/image.png')) }}"
                                alt="Slide Image">
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-4">
                                ...
                                <form method="POST" action="{{ route('admin.activities.toggle', $item->id) }}">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                ...
                                <form method="POST" action="{{ route('admin.activities.destroy', $item->id) }}"
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 space-y-4">
                <div class="flex items-start gap-4">
                    <img class="h-20 w-32 object-cover rounded-md border"
                        src="{{ asset('storage/' . ($item->icon ?? 'placeholders/image.png')) }}" alt="Slide Image">
                    <div class="flex-1">
                        <h4 class="font-bold text-gray-800">{{ $item->title }}</h4>
                        <p class="text-sm text-gray-500">Serial: <span class="font-medium text-gray-700">{{
                                $item->serial }}</span></p>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center border-t pt-3">
                    <div>
                        ...
                        @else
                        <span
                            class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Inactive</span>
                        @endif
                    </div>
