...
@section('content')
<form method="POST" action="{{ @$album->id ? route('admin.albums.update', $album->id) : route('admin.albums.store') }}"
    enctype="multipart/form-data">
    <div class="flex justify-between items-center ">
        <h2 class="text-3xl pt-6 px-5 font-bold text-gray-800 mb-2">
            {{ @$album->id ? 'Edit' : 'Add' }} Album
        </h2>
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg w-full border border-gray-200">
            <div class="grid grid-cols-1 mt-6">
                <div class="lg:col-span-8 bg-white ">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b border-gray-200 pb-3">Details</h3>
                    <div class="space-y-6 pt-4">
                        <div>
                            <label for="name" class="block mb-2 text-base font-medium text-gray-700">Album Name</label>
                            <input id="name" name="name" type="text" value="{{ old('name', @$album->name) }}" required
                        ...
                        <div>
                            <label for="description" class="block mb-2 text-base font-medium text-gray-700">Short
                                Description</label>
                            <textarea id="description" name="description" rows="4"
                                class="w-full px-4 py-3 bg-white border @error('description') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">{{ old('description', @$album->description) }}</textarea>
                            @error('description')
                            <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input id="serial" name="serial" type="number"
                                    value="{{ old('serial', @$album->serial ?? 1) }}"
                            ...
                            <div>
                                ...
                                <select id="status" name="status"
                                    ...
                                    <option value="1" {{ old('status', @$album->status) != '0' ? 'selected' : ''
                                        }}>Active</option>
                                    <option value="0" {{ old('status', @$album->status) == '0' ? 'selected' : ''
                                        }}>Inactive</option>
                                </select>
                                @error('status')
                                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                ...
                <div class="lg:col-span-4 rounded-xl">
                    <div class="flex flex-col items-center text-center">
                        <div id="image-preview-container"
                            ...
                            <img id="preview" src="{{ asset('storage/' . $album->image) }}" alt="Current Image"
                        ...
                        <label for="image"
                            ...
                            <span>{{ @$album->id ? 'Change Image' : 'Select Image' }}</span>
