...
<form method="POST" action="{{ @$gallery->id ? route('admin.gallery.update', $gallery->id) : route('admin.gallery.store') }}"
    enctype="multipart/form-data">
    <div class="flex justify-between items-center ">
        <h2 class="text-3xl pt-6 px-5 font-bold text-gray-800 mb-2">
            {{ @$album->id ? 'Edit' : 'Add' }} Gallery Image
        </h2>
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg w-full border border-gray-200">
            ...
            <div class="grid grid-cols-1 mt-6">
                ...
                <div class="lg:col-span-8 bg-white ">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-3">Details</h3>
                    <div class="space-y-6">

                         <div>
                            <label for="name" class="block mb-2 text-base font-medium text-gray-700">Name / Title</label>
                            <input id="name" name="name" type="text" value="{{ old('name', @$gallery->name) }}" required
                        ...
                        <div>
                            <label for="album_id" class="block mb-2 text-base font-medium text-gray-700">Album</label>
                            <select id="album_id" name="album_id" required class="w-full px-4 py-3 bg-white border @error('album_id') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input id="serial" name="serial" type="number"
                                    value="{{ old('serial', @$gallery->serial ?? 1) }}"
                                    class="w-full px-4 py-3 bg-white border @error('serial') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm
                            ...
                            <div>
                                ...
                                <select id="status" name="status"
                                    class="w-full px-4 py-3 bg-white border @error('status') border-red-500 @else border-gray-300 @enderror
                                    rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                                    <option value="1" {{ old('status', @$gallery->status) != '0' ? 'selected' : ''
                                        }}>Active</option>
                ...
                <div class="lg:col-span-4 rounded-xl">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <label for="image"
                            ...
                            <span>{{ @$gallery->id ? 'Change Image' : 'Select Image' }}</span>
